        ...
        {['reps', 'rest', 'tempo'].map(info => {
          return(
            <div className="flex flex-col p-2 rounded border-[1.5px] border-solid border-slate-900 w-full" key={info}>
              <h3 className="text-slate-400 text-sm capitalize">{info ==='reps' ? `${exercise.unit}` : info}</h3>
              <p className="font-medium">{exercise[info]}</p>
            </div>
          )
        })}
        <button onClick={handleSetIncrement} className="flex flex-col p-2 rounded border-[1.5px] border-solid border-blue-900 duration-200 hover:border-blue-600 w-full">
